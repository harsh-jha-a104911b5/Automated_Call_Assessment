...
def generate_labeled_csv(csv_file_path, tess_model_path, output_csv_path):
    ...
    # Generate pseudo-labels using the TESS model if labels are not available
    if labels is None:
        print("No labels provided. Generating pseudo-labels using the TESS model...")
        pseudo_labels = tess_model.predict(audio_features)
        full_data['Emotion'] = pseudo_labels  # Add the predicted labels to the DataFrame
        print("Pseudo-labels generated successfully.")
    else:
        print("Labels already provided. Using the existing labels.")
...
if __name__ == "__main__":
    ...
    output_csv_path = 'D:/Final project/emotion_analysis_project/datasets/audio_dataset/custom_audio_labeled.csv'
